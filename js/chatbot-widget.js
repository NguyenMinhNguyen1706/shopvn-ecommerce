  ...
  const chatHTML = `
    <div id="shopvn-chatbot" class="chatbot-container">
      <div class="chatbot-header" onclick="toggleChat()">
        <div class="chatbot-title">Trợ lý ShopVN</div>
        <div class="chatbot-status">Sẵn sàng hỗ trợ</div>
      </div>
      <div class="chatbot-body" id="chatbot-body">
        <div class="chat-message bot">
          Xin chào! Mình là trợ lý của ShopVN. Bạn cần tư vấn sản phẩm hay hỗ trợ đơn hàng?
        </div>
      </div>
    ...
    <button class="chatbot-trigger" onclick="toggleChat()" aria-label="Mở trợ lý ShopVN">?</button>
  `;
...
window.sendChatMessage = async function() {
  ...
  try {
    ...
    document.getElementById(loadingId)?.remove();
    appendMessage('bot', 'Xin lỗi, hiện chưa thể kết nối tới máy chủ.');
  }
};
